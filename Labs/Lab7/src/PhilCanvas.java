...
class PhilCanvas extends Canvas {
    ...
    PhilCanvas(Diners controller) {
        ...
        try {
            imgs[0] = ImageIO.read(new File("image/thinking.gif"));//controller.getImage(controller.getDocumentBase(), "image/thinking.gif");
            mt.addImage(imgs[0], 0);
            imgs[1] = ImageIO.read(new File("image/hungry.gif"));//controller.getImage(controller.getDocumentBase(), "image/hungry.gif");
            mt.addImage(imgs[1], 1);
            imgs[2] = ImageIO.read(new File("image/gotright.gif"));//controller.getImage(controller.getDocumentBase(), "image/gotright.gif");
            mt.addImage(imgs[2], 2);
            imgs[3] = ImageIO.read(new File("image/eating.gif"));//controller.getImage(controller.getDocumentBase(), "image/eating.gif");
            mt.addImage(imgs[3], 3);
            imgs[4] = ImageIO.read(new File("image/gotleft.gif"));//controller.getImage(controller.getDocumentBase(), "image/gotleft.gif");
    ...
    void backdrop() {
	    ...
	    if ((offscreen == null) || (d.width != offscreensize.width)
	        ...
	        offscreen = createImage(300,320);//d.width, d.height);
	        offscreensize = d;
   ...
   void philPaint(Graphics g,int i) {
    ...
    boolean deadlocked(){
        ...
        return Fork.getHeldForkCount()==NUMPHILS;
    }
